"use client"

import React, { useState ,useEffect } from 'react'
import { useDrag, useDrop } from 'react-dnd';
import { NativeTypes } from 'react-dnd-html5-backend';
import { FaFile } from "react-icons/fa";
import { CiPause1 } from "react-icons/ci";
import { IoTrashOutline } from "react-icons/io5";
import { FaCloudUploadAlt } from "react-icons/fa";


const Filedrop = () => {

    const [droppedFiles, setDroppedFiles] = useState([]);

    const [, drop] = useDrop({
        accept: [NativeTypes.FILE],
        drop: (item, monitor) => {
            if (item.files) {
                setDroppedFiles([...droppedFiles,...item.files]);
            }
        },
    });

    const handleFileInputChange = (e) => {
        const inputFiles = e.target.files;
        setDroppedFiles([...droppedFiles, ...inputFiles]);
    };

    useEffect(() => {
        console.log('Dropped Files:', droppedFiles);
    }, [droppedFiles]);


    return (
        <>
            <section className="container p-4 mt-4">

                <div className="tt mb-2 flex items-center justify-between">
                    <div className="tt-title">
                        <h4 className="text-white  font-semibold text-xl">
                            Upload Project Files
                        </h4>
                        <p className="text-sm text-white">
                            Project files will be made available to all team members
                        </p>
                    </div>
                    <div className="tt-btn">
                        <button disabled type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded text-sm px-5 py-2.5 text-center me-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 inline-flex items-center">
                                <FaCloudUploadAlt />
                                <span className={` mx-3 `}>Upload</span>
                            {/* <svg aria-hidden="true" role="status" class="inline w-4 h-4 me-3 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#E5E7EB"/>
                            <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentColor"/>
                            </svg>
                            Upload */}
                        </button>
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-8 mt-2" >

                    <div className="col-1 file-upload">
                        <div
                            ref={drop}
                            className="flex items-center justify-center w-ful">
                            <label
                                htmlFor="dropzone-file"
                                className="flex flex-col items-center justify-center w-full h-80 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600"
                            >
                                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                    <svg
                                        className="w-8 h-8 mb-4 text-gray-500 dark:text-gray-400"
                                        aria-hidden="true"
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="none"
                                        viewBox="0 0 20 16"
                                    >
                                        <path
                                            stroke="currentColor"
                                            strokeLinecap="round"
                                            strokeLinejoin="round"
                                            strokeWidth={2}
                                            d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2"
                                        />
                                    </svg>
                                    <p className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                                        <span className="font-semibold">Click to upload</span> or drag and drop
                                    </p>
                                    <p className="text-xs text-gray-500 dark:text-gray-400">
                                        SVG, PNG, JPG or GIF (MAX. 800x400px)
                                    </p>
                                </div>
                                    <input
                                        type="file"
                                        id="dropzone-file"
                                        className="hidden"
                                        onChange={handleFileInputChange}
                                    />
                                </label>
                        </div>
                    </div>

                    <div className="col-2 file-progress">
                        <div className="tt">
                            <h2 className="text-white">
                                Files Uploaded
                            </h2>
                        </div>
                        <div className="progress-fields mt-5">
                            <div className="uploading-cont">
                                {/* File Uploading Progress Form */}
                                <ul className="text-white">
                                    {droppedFiles.map((file, index) => (
                                        <div>
                                        {/* Uploading File Content */}
                                        <div className="mb-2 flex justify-between items-center">
                                            <div className="flex items-center gap-x-3">
                                                <span className="size-8 flex justify-center items-center border border-gray-200 text-gray-500 rounded-lg dark:border-neutral-700">
                                                    <FaFile />
                                                </span>
                                                <div>
                                                    <p className="text-sm font-medium text-gray-800 dark:text-white">
                                                        <li key={index}>{file.name}</li>
                                                    </p>
                                                    <p className="text-xs text-gray-500 dark:text-gray-500">7 KB</p>
                                                </div>
                                            </div>
                                            <div className="inline-flex items-center gap-x-2">
                                                <a className="text-gray-500 hover:text-gray-100" href="#">
                                                    <CiPause1 />
                                                </a>
                                                <a className="text-gray-500 hover:text-gray-100" href="#">
                                                    <IoTrashOutline />
                                                </a>
                                            </div>
                                        </div>
                                        {/* End Uploading File Content */}
                                        {/* Progress Bar */}
                                        <div className="flex items-center gap-x-3 whitespace-nowrap">
                                            <div
                                                className="flex w-full h-2 bg-gray-200 rounded-full overflow-hidden dark:bg-gray-700"
                                                role="progressbar"
                                                aria-valuenow={100}
                                                aria-valuemin={0}
                                                aria-valuemax={100}
                                            >
                                                <div
                                                    className="flex flex-col justify-center rounded-full overflow-hidden bg-green-600 text-xs text-white text-center whitespace-nowrap transition duration-500 dark:bg-blue-500"
                                                    style={{ width: "100%" }}
                                                />
                                            </div>
                                            <div className="w-6 text-end">
                                                <span className="text-sm text-gray-800 dark:text-white">100%</span>
                                            </div>
                                        </div>
                                        {/* End Progress Bar */}
                                        </div>
                                    // {/* End File Uploading Progress Form */}
                                   
                                    ))}
                                </ul>
                                
                            </div>
                        </div>
                    </div>
                </div>
            </section>

        </>
    )
}

export default Filedrop